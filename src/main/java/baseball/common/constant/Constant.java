package baseball.common.constant;

public class Constant {
    /**
     * 초기화
      */
    public static int ANSWER_SIZE = 3;
    public static int START_RANGE = 1;
    public static int END_RANGE = 9;

    public static int NORMAL_INPUT_LENGTH = 3;
    public static String INPUT_NUMBER_STRING = "숫자를 입력해주세요 : ";

    /**
     * 게임 결과
      */
    public static boolean SUCCESS_BOOLEAN = true;
    public static int NOTHING = 0;
    public static int SUCCESS_STRIKE = 3;
    public static String NOTHING_STRING = "낫싱";
    public static String BALL_STRING = "볼";
    public static String STRIKE_STRING = "스트라이크";
    public static String SUCCESS_RESULT = SUCCESS_STRIKE + STRIKE_STRING;
    public static String SUCCESS_STRING = "3개의 숫자를 모두 맞히셨습니다! 게임 종료";

    /**
     * 재시작
      */
    public static String RESTART_INPUT_STRING = "1";
    public static String EXIT_INPUT_STRING = "2";

    public static int RESTART_INPUT_LENGTH = 1;
    public static String RESTART_MESSAGE = "게임을 새로 시작하려면 1, 종료하려면 2를 입력하세요.";

    /**
     * 예외
      */
    // 게임 진행
    public static String WRONG_LENGTH_INPUT_ERROR = "잘못된 길이의 입력입니다.";
    public static String WRONG_TYPE_INPUT_ERROR = "잘못된 타입의 입력입니다.";
    public static String WRONG_SAME_INPUT_ERROR = "서로 다른 숫자를 입력해야 합니다.";

    // 게임 종료 후 입력
    public static String WRONG_INPUT_ERROR = "잘못된 입력입니다.";
}
